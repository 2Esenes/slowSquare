...
public static class NativeShare
{
    ...
    public static void Share(string body, string filePath = null, string url = null, string subject = "",
...
#if UNITY_ANDROID
    public static void ShareAndroid(string body, string subject, string url, string[] filePaths, string mimeType,
        bool chooser, string chooserText)
    {
        Debug.Log("ShareAndroid:: " + body + ", " + subject + ", " + url + ", " + (filePaths != null) + ", " +
                  mimeType + ", " + chooser + ", " + chooserText);
        ...
        using (var intentObject = new AndroidJavaObject("android.content.Intent"))
        {
            ...
            else
            {
                ...
                {
                    var fpClass = new AndroidJavaClass("androidx.core.content.FileProvider");
                    var fileObject = new AndroidJavaObject("java.io.File", new object[] {filePaths[0]});
                    var uriObject = fpClass.CallStatic<AndroidJavaObject>("getUriForFile",
                        new object[] {currentActivity, TTPUtils.BundleIdentifier + ".fileprovider", fileObject});
