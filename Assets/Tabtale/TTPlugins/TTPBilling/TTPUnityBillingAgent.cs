...
#region IStoreListener implementation
            ...
            foreach (TTPIProduct product in _storeController.products.all)
            {
                ...
                if (IsNoAdsItem(product.definition.id))
                {
                    if (product.hasReceipt)
                    {
                        ...
                        {
                            UIAP.SubscriptionInfo itemInfo = GetSubscriptionInfo(product.definition.id);
                            if (itemInfo != null)
                            {
        ...
        public TTPIPurchaseProcessingResult ProcessPurchase(TTPIPurchaseEventArgs e)
        {
            ...
            {
                UIAP.SubscriptionInfo subscriptionInfo = GetSubscriptionInfo(e.purchasedProduct.definition.id);
            ...
            if (e.purchasedProduct.definition.type == TTPIProductType.Subscription)
            {
                ...
                UIAP.SubscriptionInfo ttpSubscriptionInfo = GetSubscriptionInfo(e.purchasedProduct.definition.id);
        ...
        public UIAP.SubscriptionInfo GetSubscriptionInfo(string itemId)
        {
            string finalId = GetFinalId(itemId);
            if (!IsInitialisedWithLogging())
                return null;
